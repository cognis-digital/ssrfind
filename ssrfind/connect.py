"""Native cognis-connect emit for ssrfind — forward findings to any platform.

Maps ssrfind's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ssrfind ... --format json | ssrfind-emit --to stix
    ssrfind-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('rule_id', '') + ' - ' + rec.get('reason', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'ssrf')
